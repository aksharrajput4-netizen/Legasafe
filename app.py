...
# ── Plotly dark theme matching brand ────────────────────────────────────────
CHART_LAYOUT = dict(
...
<style>
  ...
  .stSubheader {{ color: {TEXT} !important; }}
...
if uploaded_file:
    ...
    col4.markdown(f"<div class='score-label'>Health Score</div><div class='{score_cls}'>{score}<span style='font-size:1rem'>/100</span></div>", unsafe_allow_html=True)
    ...
    if subs:
        ...
        st.markdown(f"<p style='color:{AMBER};font-weight:700;margin-top:10px;'>Total subscription burn: ₹{total_yearly:,.0f}/year</p>", unsafe_allow_html=True)

    # ── 4. Category Breakdown — Plotly bar chart ─────────────────────────────
    if cats:
        st.divider()
        st.subheader("🗂️ Spending by Category")
        cat_df = (
            pd.DataFrame(list(cats.items()), columns=["Category", "Amount"])
            .sort_values("Amount", ascending=True)  # ascending so biggest is on top
        )
        # Gold gradient: darker for smaller, brighter for larger
        n = len(cat_df)
        colors = [
    ...
    # ── 5. Monthly Trend — Plotly line chart ─────────────────────────────────
    if monthly:
        st.divider()
    ...
    if txns:
        ...
        ecol1.download_button("📄 All Transactions", tx_csv, "legasafe_transactions.csv", "text/csv", use_container_width=True)

    if sub_rows:
        subs_csv = pd.DataFrame(sub_rows).to_csv(index=False).encode('utf-8')
        ecol2.download_button("📋 Subscriptions", subs_csv, "legasafe_subscriptions.csv", "text/csv", use_container_width=True)

    summary_csv = pd.DataFrame({
        "Metric": ["Total Spent", "Transactions", "Active Subscriptions", "Health Score", "Yearly Subscription Cost"],
        "Value":  [
    ...
    ecol3.download_button("📊 Summary Report", summary_csv, "legasafe_summary.csv", "text/csv", use_container_width=True)
    ...
    # ── Empty state ───────────────────────────────────────────────────────────
    st.markdown(f"<p style='color:{TEXT_MUTED};'>Upload your bank statement PDF above to get started. No account needed.</p>", unsafe_allow_html=True)
